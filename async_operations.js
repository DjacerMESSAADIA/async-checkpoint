...
const awaitCall = async (timeout = 1000) => {
    ...
    try {
        ...
        const errorMessage = message === 'Request timed out' 
...
const concurrentRequests = async (timeout = 2000) => {
    ...
    const createRequest = (name, delay) => 
        new Promise(resolve => setTimeout(() => resolve(`${name} API Response`), delay));
...
const parallelCalls = async (urls, { 
    ...
    try {
        ...
        for (const [i, batch] of [...Array(Math.ceil(urls.length / batchSize))].entries()) {
